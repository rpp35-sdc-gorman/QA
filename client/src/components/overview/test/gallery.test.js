...
describe("Gallery Unit Tests", () => {
    ...
    it('Should change the displayed Image when increment is clicked', () => {
      // render
      act(() => {
        createRoot(container).render(<Gallery images={testPhotos} ClickTracker={testFunction} />);
      })
    ...
    it('Should handle be able to jump to a photo with mini gallery', () => {
        // render
        act(() => {
        createRoot(container).render(<Gallery images={testPhotos} ClickTracker={testFunction} />);
        })
        // snapshot before event
        const before = container.querySelector('.Gallery').style._values['background-image']
        const thumbnails = document.querySelectorAll('.Gallery_mini')
        expect(thumbnails.length).toEqual(2)
        act(() => {
        Simulate.click(thumbnails[1])
        })
        const after = container.querySelector('.Gallery').style._values['background-image']
        expect(after).toBe(`url(${testPhotos[1].url})`)
    });
})